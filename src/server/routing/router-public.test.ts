  ...
  describe('zaak status endpoint', () => {
    test('redirects to login with digid if not authenticated', async () => {
      ...
      expect(resMock.redirect).toHaveBeenCalledWith(
        'http://bff-api-host/api/v1/auth/digid/login?id=Z%2F000%2F000001&thema=vergunningen&returnTo=%2Fzaak-status'
      );
    });
    ...
    test('redirects to login with eherkenning if not authenticated', async () => {
      ...
      expect(resMock.redirect).toHaveBeenCalledWith(
        'http://bff-api-host/api/v1/auth/eherkenning/login?id=Z%2F000%2F000001&thema=vergunningen&returnTo=%2Fzaak-status'
      );
    });
  });
});
