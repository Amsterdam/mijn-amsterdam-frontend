...
describe('Regressietest fetchBedAndBreakfast personen', () => {
  const PowerBrowserPersonenRequests = [
    ...
    {
      req: {
        url: 'https://acc_gemeenteamsterdam_vth.moverheid.nl/api/Link/PERSONEN/GFO_ZAKEN/',
        method: 'POST',
      },
      res: [
        {
  ...
  test('should fetch BB zaken successfully', async () => {
    for (const r of PowerBrowserPersonenRequests) {
      ...
      (remoteApi as any)[method](url).query(true).reply(200, r.res);
    }
...
describe('fetchBB fetchBedAndBreakfast maatschap', () => {
  const PowerBrowserMaatschapRequests = [
    ...
    {
      req: {
        url: 'https://acc_gemeenteamsterdam_vth.moverheid.nl/api/Link/MAATSCHAP/GFO_ZAKEN/',
        method: 'POST',
      },
      res: [
        {
