...
import { AV_RTM_DEEL1, AV_RTM_DEEL2 } from './regeling-rtm';
...
function createStep(
  ...
  return {
    ...
    isActive: false,
    isChecked:
      status === 'Einde recht' && aanvraag.datumEindeGeldigheid
        ? true
        : status !== 'Einde recht',
  ...
  for (const [betrokkene, aanvragen] of aanvragenByBetrokkenen.entries()) {
    ...
    const steps = aanvragen.flatMap((aanvraag, index, aanvragen) => {
      ...
      const aanvraagStatus =
        ...
        aanvragen
          .slice(0, index)
          .some((prevA) => prevA.productIdentificatie === AV_RTM_DEEL2)
          ? 'Wijzigingsaanvraag'
          : 'Aanvraag';
    ...
    if (steps.length) {
      if (toegewezenRTM && !toegewezenRTM.datumEindeGeldigheid) {
        steps.at(-2)!.isActive = true;
      } else {
      ...
      const title = aanvragen.at(-1)?.titel || 'RTM Regeling';
