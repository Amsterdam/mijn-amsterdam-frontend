...
# Copy the env appropriate .env file
COPY env.${BUILD_ENV} /app/env.production

# Some conditional setup
RUN if [ "$BUILD_ENV" = "production" ]; then rm /app/public/robots.txt
RUN npm run build
RUN echo "date=`date`; build=${BUILD_NUMBER}; build_env=${BUILD_ENV}; see also: https://github.com/Amsterdam/mijn-amsterdam-frontend/commit/${COMMIT_HASH}" > /app/build/version.txt
