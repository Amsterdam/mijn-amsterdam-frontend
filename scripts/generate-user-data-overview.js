...
function getServiceResults(fromDisk = false) {
    return __awaiter(this, void 0, void 0, function* () {
        if (fromDisk) {
            const data = JSON.parse(fs.readFileSync('./user-data.json', 'utf8').toString());
            // console.log(data);
            return data;
        }
        const allResults = {};
        for (const [Username, userId] of testAccountEntries) {
            ...
            try {
                const serviceResults = yield fetch(url).then((r) => {
                    var _a;
                    const Cookie = (_a = r.headers.get('Set-Cookie')) !== null && _a !== void 0 ? _a : '';
...
function naam(persoon) {
    var _a;
    return (_a = persoon.voornamen) === null || _a === void 0 ? void 0 : _a.split(' ').map((naam, index) => {
        return index === 0 ? naam : naam.charAt(0) + '.';
    }).join(' ');
}
function oudersOfKinderen(oudersOfKinderen, serviceResults) {
    return (oudersOfKinderen === null || oudersOfKinderen === void 0 ? void 0 : oudersOfKinderen.length)
        ? oudersOfKinderen
            .map((persoon) => relatedUser(persoon))
            .join(', ')
        : '';
}
function relatedUser(persoon) {
    var _a;
    const relatedUsername = (_a = testAccountEntries.find(([, bsn]) => bsn === persoon.bsn)) === null || _a === void 0 ? void 0 : _a[0];
    const age = `${persoon.overlijdensdatum
        ? 'overleden'
        : persoon.geboortedatum
            ? (0, date_fns_1.differenceInYears)(new Date(), (0, date_fns_1.parseISO)(persoon.geboortedatum))
            : '??'}`;
    const nom = naam(persoon);
    const user = (persoon === null || persoon === void 0 ? void 0 : persoon.bsn)
        ? `[${persoon === null || persoon === void 0 ? void 0 : persoon.bsn}${relatedUsername ? '/' + relatedUsername : ''}]`
        : '';
    return `${nom} (${age}) ${user}`;
}
function woonplaatsNaamBuitenAmsterdam(adres) {
    const woonplaatsNaam = adres === null || adres === void 0 ? void 0 : adres.woonplaatsNaam;
...
const paths = [
    ...
    },
    {
        ...
        transform: (persoon) => {
            return (`${(persoon === null || persoon === void 0 ? void 0 : persoon.voorvoegselGeslachtsnaam)
                ? persoon.voorvoegselGeslachtsnaam + ' '
                : ''}${persoon === null || persoon === void 0 ? void 0 : persoon.geslachtsnaam}` +
                ((persoon === null || persoon === void 0 ? void 0 : persoon.omschrijvingAdellijkeTitel)
    ...
    },
    {
        ...
        transform: (value) => {
            return value !== null ? (0, helpers_1.defaultDateFormat)(value) : 'onbekend';
        },
    },
    {
        label: 'Leeftijd',
        ...
        transform: (value, serviceResults) => {
            const age = value !== null
                ? (0, date_fns_1.differenceInYears)(new Date(), (0, date_fns_1.parseISO)(value)) + ''
                : 'onbekend';
            return age;
        },
    },
    {
        ...
        transform: (value, serviceResults) => {
            ...
            return ((0, helpers_1.getFullAddress)(value) +
                `${((_b = (_a = serviceResults.BRP.content) === null || _a === void 0 ? void 0 : _a.persoon) === null || _b === void 0 ? void 0 : _b.adresInOnderzoek)
                    ? ' (In onderzoek)'
                    : ''}` +
                `${((_d = (_c = serviceResults.BRP.content) === null || _c === void 0 ? void 0 : _c.persoon) === null || _d === void 0 ? void 0 : _d.vertrokkenOnbekendWaarheen)
                    ? ' (VOW)'
                    : ''}` +
                `${((_f = (_e = serviceResults.BRP.content) === null || _e === void 0 ? void 0 : _e.persoon) === null || _f === void 0 ? void 0 : _f.indicateGeheim) ? ' (Geheim)' : ''}`);
        },
    },
    {
        ...
        transform: (postcode, serviceResults) => {
            var _a;
            return `${postcode ? postcode : ''} ${woonplaatsNaamBuitenAmsterdam((_a = serviceResults.BRP.content) === null || _a === void 0 ? void 0 : _a.adres)}`;
        },
    },
    {
        label: 'Verbintenis',
        ...
        transform: (verbintenis) => {
            ...
            return verbintenis.persoon
                ? `${(_a = (verbintenis.soortVerbintenisOmschrijving ||
                    verbintenis.soortVerbintenis)) !== null && _a !== void 0 ? _a : ''} met ${relatedUser(verbintenis.persoon)}`
    ...
    },
    {
        ...
        transform: (verbintenisHistorisch) => {
            ...
            return verbintenisHistorisch
                ...
                var _a;
                return verbintenis.persoon
                    ? `${(_a = verbintenis.soortVerbintenisOmschrijving) !== null && _a !== void 0 ? _a : ''} met ${relatedUser(verbintenis.persoon)}`
                    : Object.keys(verbintenis).length
                        ? JSON.stringify(verbintenis)
                        : '';
            })
    ...
    },
    {
        ...
        transform: (adressen, serviceResults) => {
            return adressen === null || adressen === void 0 ? void 0 : adressen.map((adres) => {
                return `${(0, helpers_1.getFullAddress)(adres)} ${woonplaatsNaamBuitenAmsterdam(adres)}`;
            }).join(', ');
        },
        wch: 80,
    },
    ...
    },
    {
        ...
        transform: (value) => {
            return value === null || value === void 0 ? void 0 : value.map(({ omschrijving }) => omschrijving).join(', ');
        },
    },
].map((p) => {
    if (!p.hpx) {
...
const chaptersAvailable = menuItems_1.chaptersByProfileType.private.map((menuItem) => menuItem.id);
function getUserChapters(serviceResults) {
    ...
    const items = chapterItems.filter((item) => {
        ...
        return (item.isAlwaysVisible || (0, useChapters_helpers_1.isChapterActive)(item, serviceResults));
    });
    return Object.fromEntries(items.map((menuItem) => [menuItem.id, menuItem.title]));
}
...
function getNotificationRows(resultsByUser) {
    const rows = Object.entries(resultsByUser).flatMap(([Username, serviceResults]) => {
...
function addSheets(workbook, sheets) {
    for (const sheet of sheets) {
        ...
        if (columnHeaders === null || columnHeaders === void 0 ? void 0 : columnHeaders.length) {
...
function sheetBrpBase(resultsByUser) {
    // BRP gegevens
    const rowValues = Object.entries(resultsByUser).map(([Username, serviceResults]) => {
        const add = {
            Username: Username,
        };
    ...
    const rowInfo = Object.keys(auth_development_1.testAccounts).map(() => ({ hpx: HPX_DEFAULT }));
    const colInfo = [
        { wch: WCH_DEFAULT },
        ...paths.map((pathObj) => { var _a; return ({ wch: (_a = pathObj.wch) !== null && _a !== void 0 ? _a : WCH_DEFAULT }); }),
    ];
    return {
...
function sheetChapters(resultsByUser) {
    ...
    return {
        ...
        colInfo: [
            { wch: WCH_DEFAULT },
            ...Object.keys(auth_development_1.testAccounts).map(() => ({ wch: WCH_DEFAULT })),
        ],
        rowInfo,
    };
}
...
function sheetChapterContent(resultsByUser) {
    function count(chapter) {
        return (serviceResults) => { var _a, _b; return ((_b = (_a = serviceResults[chapter]) === null || _a === void 0 ? void 0 : _a.content) === null || _b === void 0 ? void 0 : _b.length) || ''; };
    }
    const chapterContents = {
        Burgerzaken: (serviceResults) => {
            var _a;
            return ((_a = serviceResults.BRP.content) === null || _a === void 0 ? void 0 : _a.identiteitsbewijzen.length) || '';
        },
        Bezwaren: count('BEZWAREN'),
        Bijstandaanvragen: count('WPI_AANVRAGEN'),
        Jaaropgaves: (serviceResults) => {
            var _a;
            return ((_a = serviceResults.WPI_SPECIFICATIES.content) === null || _a === void 0 ? void 0 : _a.jaaropgaven.length) || '';
        },
        Uitkeringsspecificaties: (serviceResults) => {
            var _a;
            return (((_a = serviceResults.WPI_SPECIFICATIES.content) === null || _a === void 0 ? void 0 : _a.uitkeringsspecificaties.length) || '');
        },
        Tozo: count('WPI_TOZO'),
        Tonk: count('WPI_TONK'),
        BBZ: count('WPI_BBZ'),
        'Stadspassen (Gpass)': (serviceResults) => {
            var _a;
            return ((_a = serviceResults.WPI_STADSPAS.content) === null || _a === void 0 ? void 0 : _a.stadspassen.length) || '';
        },
        Stadspasaanvragen: (serviceResults) => {
            var _a;
            return ((_a = serviceResults.WPI_STADSPAS.content) === null || _a === void 0 ? void 0 : _a.aanvragen.length) || '';
        },
        ...
        'ToerVerh Registraties': (serviceResults) => {
            var _a;
            return (((_a = serviceResults.TOERISTISCHE_VERHUUR.content) === null || _a === void 0 ? void 0 : _a.registraties.length) || '');
        },
        'ToerVerh Vergunningen': (serviceResults) => {
            var _a;
            return (((_a = serviceResults.TOERISTISCHE_VERHUUR.content) === null || _a === void 0 ? void 0 : _a.vergunningen.length) || '');
        },
        // KREFIA: (serviceResults: ServiceResults) => { return},
        Klachten: (serviceResults) => {
            var _a;
            return ((_a = serviceResults.KLACHTEN.content) === null || _a === void 0 ? void 0 : _a.aantal) || '';
        },
        Horeca: count('HORECA'),
        AVG: (serviceResults) => {
            var _a;
            return ((_a = serviceResults.AVG.content) === null || _a === void 0 ? void 0 : _a.verzoeken.length) || '';
        },
        'Bodem (Loodmeting)': (serviceResults) => {
            var _a;
            return ((_a = serviceResults.BODEM.content) === null || _a === void 0 ? void 0 : _a.metingen.length) || '';
        },
    };
    const results = Object.entries(resultsByUser).map(([Username, serviceResults]) => {
        const base = {
            Username,
        };
...
getServiceResults(fromDisk).then((resultsByUser) => {
    ...
    const fileName = `Test-Data-ACC-${(0, helpers_1.dateFormat)(new Date(), 'yyyy-MM-dd')}.xlsx`;
