...
git branch -m $NEWTAG && git tag -a $NEWTAG -m $NEWTAG && npm --no-git-tag-version version "$MAJ.$MIN.$BUG" && git add package.json package-lock.json && git commit -m "Bump! $NEWTAG" &&
